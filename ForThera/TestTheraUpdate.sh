...
if [ ! -f "/home/odroid/.config/testupdate10212020" ]; then
	printf "\nAdd option to use Mednafen_PCE or Mednafen_supergrafx retroarch core for increased accuracy and compatibility\n" | tee -a "$LOG_FILE"
	sudo wget https://github.com/christianhaitian/rgb10/raw/master/ForThera/mednafen_pce/mednafen_pce_libretro.so -O /home/odroid/.config/retroarch/cores/mednafen_pce_libretro.so -a "$LOG_FILE"
	...
	sudo mv -v /etc/emulationstation/es_systems.cfg /etc/emulationstation/es_systems.cfg.update$UPDATE_DATE.bak | tee -a "$LOG_FILE"
	sudo wget https://github.com/christianhaitian/rgb10/raw/master/ForThera/mednafen_pce/es_systems.cfg -O /etc/emulationstation/es_systems.cfg -a "$LOG_FILE"
	sudo chown -v odroid:odroid /etc/emulationstation/es_systems.cfg | tee -a "$LOG_FILE"
	...
	touch "/home/odroid/.config/testupdate10212020"
	printf "\033c" >> /dev/tty1
fi
...
if [ ! -f "/home/odroid/.config/testupdate10272020" ]; then
	...
	sudo wget https://github.com/christianhaitian/rgb10/raw/master/ForThera/mupen64plus-standalone/InputAutoCfg.ini -O /opt/mupen64plus/InputAutoCfg.ini -a "$LOG_FILE"
	sudo wget https://github.com/christianhaitian/rgb10/raw/master/ForThera/mupen64plus-standalone/es_systems.cfg -O /etc/emulationstation/es_systems.cfg -a "$LOG_FILE"
	sudo wget https://github.com/christianhaitian/rgb10/raw/master/ForThera/mupen64plus-standalone/mupen64plus-input-sdl.so -O /opt/mupen64plus/mupen64plus-input-sdl.so -a "$LOG_FILE"
	sudo wget https://github.com/christianhaitian/rgb10/raw/master/ForThera/mupen64plus-standalone/mupen64plus.cfg -O /home/odroid/.config/mupen64plus/mupen64plus.cfg -a "$LOG_FILE"
	sudo wget https://github.com/christianhaitian/rgb10/raw/master/ForThera/mupen64plus-standalone/n64.sh -O /usr/local/bin/n64.sh -a "$LOG_FILE"
...
if [ ! -f "$UPDATE_DONE" ]; then
	...
	sudo wget https://github.com/christianhaitian/rgb10/raw/master/ForThera/mupen64plus-standalone/libmupen64plus.so.2.0.0 -O /opt/mupen64plus/libmupen64plus.so.2.0.0 -a "$LOG_FILE"
	sudo wget http://eple.us/retroroller/libretro/aarch64/o2em_libretro.so.zip -a "$LOG_FILE"
	sudo wget https://github.com/christianhaitian/rgb10/raw/master/ForThera/o2em/addo2em.txt -a "$LOG_FILE"
	...
	msgbox "Added support for pausing in standalone mupen64plus emulator using Minus and A button combination and add Odyssey2 emulator support."
	touch "$UPDATE_DONE"
